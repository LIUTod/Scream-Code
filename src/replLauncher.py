...
from .repl_utils import *  # noqa: F401,F403
...
class _StreamingTurnSession:
    ...
    def _request_tool_approval_sync(self, tool_name: str, tool_args: str) -> bool:
        ...
        while True:
            ...
            if ans in ('n', 'no', ''):
                break
            self.console.print(
                '[yellow]⚠️ 无效输入。请输入 [bold]Y[/bold] (允许)、[bold]A[/bold] (全局放行) 或 [bold]N[/bold] (拒绝)。[/yellow]'
            )
    ...
    def _render_finish_turn_success(self, ev: dict[str, Any]) -> None:
        ...
        # 仅当输出为空时打印完成指示，避免有实质内容时冗余提示黏贴到输入框上方
        has_output = bool(body) or show_tool_collapse
        if not has_output:
            self.console.print(
                '[bold green]✓ 输出完成[/bold green] '
                '[dim](按 Enter 继续)[/dim]'
            )
            self.console.print()
        ...
        try:
            ...
            while pending is not None:
                ...
                if et == 'api_tool_op':
                    ...
                    self._squash_live_for_halt()
                    self._stop_live()
                    # 将 chunker 中尚未静态落盘的尾部 flush 出来，避免 Live 擦除后丢失；
                    # 不再用 _print_assistant_output 打印整段 buffer（chunker 已在流式中落盘过）。
            ...
            while pending is not None:
                ...
                if et == 'api_tool_op':
                    ...
                    if self._is_sensitive_tool(tool_name):
                        ...
                        # 审批循环：无效输入时重新创建 future，避免一次错误输入导致轮次卡死。
                        approval_answer: str = ''
                        while True:
                            ...
                            try:
                                answer = await asyncio.wait_for(
                                    self.approval_future, timeout=600.0
                                )
                            except asyncio.TimeoutError:
                                self.console.print(
                                    '[yellow]⚠️ 审批超时（10 分钟），默认拒绝。[/yellow]'
                                )
                                answer = 'n'
                            ...
                            if ans in ('y', 'yes'):
                                ...
                                # 循环继续，重新创建 future 等待下一轮输入

                        if approval_answer in ('n', 'no', ''):
                            try:
                                self.engine.request_stream_abort()
                            except Exception:
                                pass
    ...
    async def _cancel_active_prompt_task() -> None:
        ...
        if not t.done():
            ...
            try:
                ...
                pass
        # 取消任务即可让 prompt_async 退出；不再强制 app.exit，避免与 prompt_toolkit
        # 内部状态机冲突（Return value already set / 残影等）。
        try:
            await t
